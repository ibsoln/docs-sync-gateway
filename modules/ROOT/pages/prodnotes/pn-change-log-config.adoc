...
The SG-Replicate 1.0 global level `replications` configuration property is deprecated at his release.

SG-Replicate 2.0 replications are now defined with a `replications` property at the individual database level; that is, as a member of the `database.this_db` properties.
...
The following SG Replicate 1.0 (SG1) _ReplicationConfig_ configuration properties are deprecated.
...
+
*For SG2:* the _source_ property is, implicitly, the database associated with the replication (that is, the 'database' property under which it is defined).
// end::source[]
...
__SG 1.0 Deprecated__.
+
*For SG2:* The _target_ property is replaced by the `<<remote-property>>` property.
// end::target[]

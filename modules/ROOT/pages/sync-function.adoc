...
`meta`::
This object references the access grant xattr identifying the channel (or channels) we should route the current document to.
You can see a simple example of its use in <<ex-xattr-access-grant>>.
+
NOTE: This is only available if the following configuration settings are in place: {configuration-schema-database--xref-enable-shared-bucket-access} is `true` and {configuration-schema-database--xref-user-xattr-key} is set to a valid xattr name.

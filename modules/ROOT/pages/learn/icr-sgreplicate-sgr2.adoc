...
:page-role:
:description: Inter-sync-gateway replication (Sync Gateway to Sync Gateway) using SG-Replicate 2.0 protocol
...
:param-abstract: This content provides an overview of Inter-sync-gateway replication, which is replication between Sync Gateway clusters using SG-Replicate 2.0.
...
NOTE: This content covers inter-sync-gateway replication using the SG-Replicate 2.0 protocol (_SGR-2_).

== Replication Characteristics
_In this section_:  <<context>>  |  <<protocol>>  |  <<types-of-replication>>  |  <<granularity>>  |  <<directionality>>  |  <<security>>
...
=== Context
Sync Gateway supports the ability to run replications between Sync Gateway clusters using SG-Replicate 2.0 (SGR-2), with the replicator {glos-term-synchronizing} changes between two {glos-term-sync-gateway-databases}.
...
Sync Gateway node can opt-out of participating in the replication process using the database-level parameter {xref-sgw-pg-config-properties-databases-sgr-enabled}.
...
====
You can also configure replications to use delta-sync, where only the changed data items are transferred.

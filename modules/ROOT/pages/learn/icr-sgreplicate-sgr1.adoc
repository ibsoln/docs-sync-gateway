...
:page-role:
:description: Inter-cluster replication (Sync Gateway to Sync Gateway) using SG-Replicate1.0 protocol
...
CAUTION: This content covers inter-cluster replication using the SG-Replicate 1.0 protocol.

== Replication Characteristics

= Protocol

Sync Gateway supports the ability to run replications between two Sync Gateway clusters using SG-Replicate 1.0 (SGR-1).
...
Under SGR01 inter-cluster replications are HTTP(s)-based.

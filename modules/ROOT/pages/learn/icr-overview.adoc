...
SG-Replicate 1.0 is deprecated as of Sync Gateway version 2.8. Unless otherwise specified replications configured and run as SG-Replicate 1.0 replications will continue to function as they did prior to version 2.8.

== Feature Summary

* SG-Replicate 2.0 offers websocket-based replications based on a secure single TCP connection delivering bi-directional, full-duplex, communication. Sync Gateway leverages the full duplex nature of websockets to support push and pull from a single connection -- see {xref-sgw-pg-config-properties-db-rep-direction}

= Inter-Cluster Replication with SG-Replicate 1.0
:page-layout: article
...
:param-abstract: This content provides an overview of Inter-cluster replication, which is replication between Sync Gateway clusters using SG-Replicate 1.0.
...
Sync Gateway's inter-cluster replication feature using the SG-Replicate 1.0  protocol. has been an integral part of Sync Gateway since xxxxxx. It act as the _{glos-term-active-replicator}_, pulling changes from a data source and pushing them to a target.
...
There is no change in high-availability behavior {fn-icr-ref} for replications run using SG-Replicate 1.0 and-or for replications run using the {community} edition.

Replications run using SG-Replicate 1.0 and-or Couchbase Mobile's {community} edition run only on the node defined in the configuration {fn-h2}.

Duplicate definitions on different nodes will result in the same replication being run on multiple nodes.
...
The content returned will be the same as that returned by `_replicationStatus` and contains additional content over the pre-2.8 version:
+
** direction
** is_persistent

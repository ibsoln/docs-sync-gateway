...
In {enterprise} the SG-Replicate 2.0 protocol automatically provides for high availability replication.
It uses <<node-distribution>> to ensure that should any given node fail the system automatically resumes the node's replications on an alternative node (providing the replication has been configured on multiple nodes).
// end::overview[]

_Action_: To make a replication a candidate for HA the user must configure it on at least two Sync Gateway nodes.

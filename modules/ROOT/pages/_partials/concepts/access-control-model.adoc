...
// tag::body[]
== Model
In the Couchbase Mobile ecosystem access to documents is governed by three key entities ({sgw--xref}{user--page}[users], {sgw--xref}{role--page}[roles] and {sgw--xref}{channels--page}[[channels]).
...
Conceptually, the “channel” can be considered as a “tag” associated with a document.
...
Instead you can explicitly implement write access controls within the Sync Function; perhaps restricting updates to specific users or roles — for more on this see {access-control-how-write-access-xref}.
...
* Channels +
The {channels--xref} topic how channels are created and how documents can be assigned to channels.

* Users +
The {user--xrf} topic discusses provisioning of users and providing users access to channels.

* Roles
The {role--xref} topic discusses roles, assignment of users to roles created and providing roles access to channels.

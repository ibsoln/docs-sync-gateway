...
// tag::in-this-section[]
*_In this Section_*: <<automatic-conflict-resolution>>  |  <<resolver-policies>>  |  <<how-resolution-works>>  |  <<configuring-a-policy-to-use>>  |  <<custom-conflict-resolution-ee>>
// end::in-this-section[]

== Introduction
// tag::overview[]
Sync Gateway's inter-sync-gateway replication supports automatic conflict resolution to resolve conflicting document changes (revisions).

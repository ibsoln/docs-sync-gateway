...
Both scenarios are covered in <<replication-properties>>, which summarizes their {glos-term-replication-definition} elements{fn-repdef-both}. These are covered in more detail in {xref-sgw-pg-config-properties}.
...
For SGR-2, _replication definitions_ are at database level -- see: <<replication-properties>>{fnref-repdef-both}.
